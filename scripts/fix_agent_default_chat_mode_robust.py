...
def patch_worker():
    ...
    if 'pathname === "/api/openai/chat"' in s or 'pathname == "/api/openai/chat"' in s:
...
'''
    ...
    raise SystemExit("Could not find stable insertion point in worker. Run: grep -n \"dashboard\\|ASSETS.fetch\\|openai\" src/worker/index.js")
...
'''
    ...
    if "Agent Connor mode tabs" not in s:
        CSS_FILE.write_text(s + add)
        print("Added CSS for agent mode tabs")
    else:
...
def main():
    ...
    run([
        "git", "add",
        "src/worker/index.js",
        "src/dashboard/pages/AgentIDE.jsx",
        "src/dashboard/dashboard.css",
        "docs/AGENT_CHAT_VS_CODE_MODE.md",
        "scripts/fix_agent_default_chat_mode_robust.py"
    ], check=False)
